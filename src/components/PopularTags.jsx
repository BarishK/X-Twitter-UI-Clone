import { Image } from "@imagekit/next";

export default function PopularTags() {
  return (
    <div className="p-4 rounded-2xl border-[1px] border-borderGray flex flex-col gap-4">
      <h1 className="text-xl font-bold text-textGrayLight">What's Happening</h1>
      {/* TREND EVENT */}
      <div className="flex gap-4">
        <div className="relative w-20 h-20 rounded-xl overflow-hidden">
          <Image
            urlEndpoint={process.env.NEXT_PUBLIC_IMAGE_KIT_ENDPOINT}
            src="public/general/cover.jpg"
            alt=""
            fill
          />
        </div>
        <div className="flex-1">
          <h2 className="font-bold text-textGrayLight">Eipstein Folders</h2>
          <span className="text-sm text-textGray">Last Night</span>
        </div>
      </div>
      {/* TOPICS */}
      <div>
        <div className="flex items-center justify-between">
          <span className="text-textGray text-sm">Technology + Trending</span>
          <Image
            urlEndpoint={process.env.NEXT_PUBLIC_IMAGE_KIT_ENDPOINT}
            src="public/icons/infoMore.svg"
            alt="info"
            width={16}
            height={16}
          />
        </div>
        <h2 className="text-textGrayLight font-bold">OpenAI</h2>
        <span className="text-textGray text-sm">20K posts</span>
      </div>
      {/* TOPICS */}
      <div>
        <div className="flex items-center justify-between">
          <span className="text-textGray text-sm">World</span>
          <Image
            urlEndpoint={process.env.NEXT_PUBLIC_IMAGE_KIT_ENDPOINT}
            src="public/icons/infoMore.svg"
            alt="info"
            width={16}
            height={16}
          />
        </div>
        <h2 className="text-textGrayLight font-bold">Epstein</h2>
        <span className="text-textGray text-sm">120K posts</span>
      </div>
      {/* TOPICS */}
      <div>
        <div className="flex items-center justify-between">
          <span className="text-textGray text-sm">U.S.A</span>
          <Image
            urlEndpoint={process.env.NEXT_PUBLIC_IMAGE_KIT_ENDPOINT}
            src="public/icons/infoMore.svg"
            alt="info"
            width={16}
            height={16}
          />
        </div>
        <h2 className="text-textGrayLight font-bold">D.Trump</h2>
        <span className="text-textGray text-sm">10K posts</span>
      </div>
      {/* TOPICS */}
      <div>
        <div className="flex items-center justify-between">
          <span className="text-textGray text-sm">Software</span>
          <Image
            urlEndpoint={process.env.NEXT_PUBLIC_IMAGE_KIT_ENDPOINT}
            src="public/icons/infoMore.svg"
            alt="info"
            width={16}
            height={16}
          />
        </div>
        <h2 className="text-textGrayLight font-bold">Javascript v2</h2>
        <span className="text-textGray text-sm">12k posts</span>
      </div>
    </div>
  );
}
